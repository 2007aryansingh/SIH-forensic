...
def save_evidence_metadata(email_id: str, evidence: EvidenceMetadata) -> None:
    ...
    if _redis_available():
        ...
        if redis_client.exists(key):
            logger.info(logfmt("evidence_metadata_exists", email_id=email_id, key=key))
...
def get_evidence_metadata(email_id: str) -> Optional[EvidenceMetadata]:
    if _redis_available():
        ...
        if not data:
            return None
        return EvidenceMetadata.model_validate(json.loads(data))

    data = _evidence_store.get(email_id)
    if not data:
        return None
    return EvidenceMetadata.model_validate(data)
...
def save_email_analysis(email_id: str, analysis: dict) -> None:
    ...
    if _redis_available():
        redis_client.set(f"email:{email_id}:analysis", payload)
        return
...
def log_report_generation(event_type: str, email_id: str, actor: str, report_id: str) -> dict:
    ...
    if _redis_available():
        ...
        _audit_logs.setdefault("report", []).append(event)
    return event
